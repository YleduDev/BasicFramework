namespace  Framework
{
    ...
    public static class LocalImageResUtil
    {
        ...
        public static bool IsLocalImageResName(this string name)
        {
            return name.ToLower().StartsWith("localimage:");
        }
    }
    ...
    public class LocalImageRes : Res
    {
        ...
        public static LocalImageRes Allocate(string name,string originalAssetName)
        {
            var res = SafeObjectPool<LocalImageRes>.Instance.Allocate();
            if (res != null)
            {
                res.AssetName = name;
                if (originalAssetName.IsLocalImageResName())
                    originalAssetName = originalAssetName.Substring(11);
                res.SetUrl(originalAssetName);
            }
            return res;
        }
