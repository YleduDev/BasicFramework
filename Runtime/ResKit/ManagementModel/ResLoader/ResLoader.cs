namespace  Framework
{
    ...
    public class ResLoader : DisposableObject, IResLoader
    {
        ...
        #region LocalAudio 本地音频

        public void LoadLocalAudioAsync(string assetName,  Action<bool, IRes> listener = null,
          bool lastOrder = true)
        {
            Add2Load(assetName, ResLoadType.LocalAudio, listener, lastOrder);

            LoadAsync();
        }

        public void LoadLocalAudioAsync(List<string> assetNames,Action callback)
        {
            if (assetNames == null)
            {
                return;
            ...
            for (var i = assetNames.Count - 1; i >= 0; --i)
            {
                var resSearchRule = ResSearchKeys.Allocate(assetNames[i], ResLoadType.LocalAudio,null, typeof(AudioClip));
        ...
        /// <summary>
        /// 同步加载音频文件，在异步加载资源后（有缓存）
        /// </summary>
        /// <param name="assetName"></param>
        /// <returns></returns>
        public AudioClip LoadLocalAudioSyncWhenAsyncFinish(string assetName)
        {
            var resSearchKeys = ResSearchKeys.Allocate(assetName, ResLoadType.LocalAudio ,null, typeof(AudioClip));
            var retAsset = LoadResSync(resSearchKeys);
            resSearchKeys.Recycle2Cache();
            return retAsset.Asset as AudioClip;
        }

        #endregion


        #region  NetAudio 网络音频

        public void LoadNetAudioAsync(string url,AudioType audioType,Action<bool, IRes> listener = null,
         bool lastOrder = true)
        {
            ResLoadType resLoadType = GetResLoadType4AudioType(audioType);

            Add2Load(url, resLoadType, listener, lastOrder);
        ...
        public void LoadNetAudioAsync(List<string> assetNames, AudioType audioType, Action callback)
        {
            if (assetNames == null)
            {
                return;
            ...
            for (var i = assetNames.Count - 1; i >= 0; --i)
            {
                var resSearchRule = ResSearchKeys.Allocate(assetNames[i], resLoadType, null, typeof(AudioClip));
        ...
        public ResLoadType GetResLoadType4AudioType(AudioType audioType)
        {
            ResLoadType resLoadType = ResLoadType.NetAudioMPEG;

            switch (audioType)
            {
                case AudioType.UNKNOWN: resLoadType = ResLoadType.NetAudioUNKNOWN; break;
                case AudioType.ACC: resLoadType = ResLoadType.NetAudioACC; break;
                case AudioType.AIFF: resLoadType = ResLoadType.NetAudioAIFF; break;
                case AudioType.IT: resLoadType = ResLoadType.NetAudioIT; break;
                case AudioType.MOD: resLoadType = ResLoadType.NetAudioMOD; break;
                case AudioType.MPEG: resLoadType = ResLoadType.NetAudioMPEG; break;
                case AudioType.OGGVORBIS: resLoadType = ResLoadType.NetAudioOGGVORBIS; break;
                case AudioType.S3M: resLoadType = ResLoadType.NetAudioS3M; break;
                case AudioType.WAV: resLoadType = ResLoadType.NetAudioWAV; break;
                case AudioType.XM: resLoadType = ResLoadType.NetAudioXM; break;
                case AudioType.XMA: resLoadType = ResLoadType.NetAudioXMA; break;
                case AudioType.VAG: resLoadType = ResLoadType.NetAudioVAG; break;
                case AudioType.AUDIOQUEUE: resLoadType = ResLoadType.NetAudioAUDIOQUEUE; break;
            }
            return resLoadType;
        }
        #endregion
        ...
        //同步加载不需要 先卸载在加载那种特殊情况（需要这种特殊情况使用异步加载）
        ...
        /// <summary>
        /// 先回收后加载 在某些情况回用到（当前已经加载了资源，后资源或ab包发生改变）
        /// </summary>
        public void Add2Load4Released(string ownerBundle, string assetName, Action<bool, IRes> listener = null,
            bool lastOrder = true)
        {
            //先 Released
            ...
            if (res == null)
            {
                Add2Load(ownerBundle, assetName, listener, lastOrder);
            }
            else
            {
               ResMgr.Instance.StartCoroutine(ToReleased(ownerBundle, assetName, listener, lastOrder));
            }
        }

        IEnumerator ToReleased(string ownerBundle, string assetName, Action<bool, IRes> listener = null,
            bool lastOrder = true)
        {          
            var resSearchRule = ResSearchKeys.Allocate(assetName, ownerBundle);
            var res = ResMgr.Instance.GetRes(resSearchRule);
            while (res != null)
            {
                ReleaseRes(assetName);
                yield return new WaitForEndOfFrame();
                res = ResMgr.Instance.GetRes(resSearchRule);
            }
            Add2Load(ownerBundle, assetName, listener, lastOrder);
        }

        IEnumerator ToReleased( string assetName, ResLoadType type, Action<bool, IRes> listener = null,
           bool lastOrder = true)
        {
            var resSearchRule = ResSearchKeys.Allocate(assetName, type);
            var res = ResMgr.Instance.GetRes(resSearchRule);
            while (res != null)
            {
                ReleaseRes(assetName);
            ...
            resSearchRule.Recycle2Cache();
            Add2Load( assetName, type, listener, lastOrder);
        }
        /// <summary>
        /// 先回收后加载 在某些情况回用到（当前已经加载了资源，后资源或ab包发生改变）
        /// </summary>
        public void Add2Load4Released(string assetName, ResLoadType type, Action<bool, IRes> listener = null,
         bool lastOrder = true)
        {
            //先 Released
            var resSearchRule = ResSearchKeys.Allocate(assetName, type);
            ...
            {
                ResMgr.Instance.StartCoroutine(ToReleased( assetName, type, listener, lastOrder));
            }
        }
        ...
        public void Add2Load( string assetName, ResLoadType type, Action<bool, IRes> listener = null,
          bool lastOrder = true)
        {
            var searchRule = ResSearchKeys.Allocate(assetName, type);
        ...
        public AsyncOperation LoadAsync(string sceneName,string ownerBundleName, LoadSceneMode loadSceneMode = LoadSceneMode.Single, System.Action listener = null)
        {
            mListener = listener;

            var resSearchRule = ResSearchKeys.Allocate(sceneName,ResLoadType.ABScene, ownerBundleName);
        ...
        {
            var resSearchRule = ResSearchKeys.Allocate(assetName,ResLoadType.ABScene , ownerBundle);
            ...
            if (ResFactory.AssetBundleSceneResCreator.Match(resSearchRule))
            {
                ...
                if (AssetBundlePathHelper.SimulationMode)
                {
                    ...
                    if (!path.IsNullOrEmpty())
                    {
                        ...
                        //UnityEditor.SceneManagement.EditorSceneManager.LoadScene(path);
