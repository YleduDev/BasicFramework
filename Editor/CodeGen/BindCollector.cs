			...
			foreach (Transform childTrans in curTrans)
			{
				...
				if (null != uiMark)
				{
					...
					if (uiMark.GetBindType() != BindType.DefaultUnityElement)
					{
						...
						SearchBinds(childTrans, transFullName + childTrans.name + "/",panelCodeInfo, elementCodeData, leafPanelType);
					}
					else
					{
                        // 如果是标记的叶子节点则不再继续搜索
                        ...
                        if (leafPanelType != null &&(uiMark.Transform.GetComponent(leafPanelType)))
                        {
							...
							SearchBinds(childTrans, transFullName + childTrans.name + "/", panelCodeInfo,
								parentElementCodeInfo, leafPanelType);
						}
					}
				}
				else
				{
					SearchBinds(childTrans, transFullName + childTrans.name + "/",panelCodeInfo, parentElementCodeInfo, leafPanelType);
				}
			}
		}
    }
